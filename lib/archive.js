...
function createArchiveStore(dir) {
  ...
  async function setSettings(cityId, raw) {
    ...
    const current = inMemory.settings[cityId] || { enabled: false, time: '', lastSavedDate: '', retentionDays: null };
    ...
    const next = { enabled, time, lastSavedDate: current.lastSavedDate || '', retentionDays };
  ...
  async function pruneCity(cityId) {
    ...
    // Retention in days: drop snapshots older than settings.retentionDays.
    ...
    // Keep only the newest MAX_ARCHIVES_PER_CITY per city.
    for (const item of citySnapshots.slice(MAX_ARCHIVES_PER_CITY)) doomed.add(item.id);

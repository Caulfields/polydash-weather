...
//
// Classification = auto-table's day-detail highlight (`detailRateHit` in
// auto-table public/js/render.js) applied to EVERY model slot:
// - take the market day's FIRST collection ("start") and the CONTROL collection
//   (last scheduled slot);
// - each has model slots: basic / additional / test / avg (mean of the base
//   slots, auto-table `buildAvgSlot`) / kma (Seoul);
// - pair = rateNumbers(slot): [roundHalfDown(todayMax), +1], null when low
//   clouds >= 50% or data missing (auto-table RATES_RULES);
// - a slot HITS when the control slot's ratesPct on ANY number of its own
//   initial pair is >= RATE_HIT_MIN (96, same constant as auto-table);
// - GREEN when ANY model slot hits (Igor, 01.09.2026), RED when slots have
//   rates but none hits, otherwise left unclassified.
...
// every model checked at classification: base slots + avg + kma (as in
// auto-table's day-detail DETAIL_SLOT_ORDER)
const CLASSIFY_SLOT_KEYS = [...BASE_SLOT_KEYS, 'avg', 'kma'];
...
function isKmaSlot(slot) {
  return !!(slot && slot.slot === 'kma');
}

// Per-model slots of one archive for classification, auto-table detailBlock
// style: base slots (with data) + avg + kma.
function classifySlots(archive, city) {
  const r = cityResult(archive, city);
  const slots = r && Array.isArray(r.slots) ? r.slots : [];
  const byKey = {};
  for (const k of BASE_SLOT_KEYS) {
    const s = slots.find((x) => x && x.slot === k) || null;
    byKey[k] = s && slotHasData(s) ? s : null;
  }
  byKey.avg = buildAvgSlot(BASE_SLOT_KEYS.map((k) => byKey[k]));
  const kma = slots.find((x) => isKmaSlot(x)) || null;
  byKey.kma = kma && slotHasData(kma) ? kma : null;
  return byKey;
}
      ...
      try {
        ...
        for (const snap of snaps) {
          ...
          if (!initial) {
            ...
            log(`${cityId} ${snap.dateKey}: no initial collection — left unclassified`);
            continue;
          }
          ...
          // check EVERY model slot (basic/additional/test/avg/kma): the
          // initial pair of a slot against the SAME slot's control rates —
          // auto-table detailRateHit per slot. Green when ANY model hits.
          const initialSlots = classifySlots(initial, city);
          const controlSlots = classifySlots(control, city);
          const attempts = [];
          for (const key of CLASSIFY_SLOT_KEYS) {
            const pair = rateNumbers(initialSlots[key]);
            if (!pair) continue;
            const cSlot = controlSlots[key];
            if (!cSlot) continue;
            const pct = cSlot.ratesPct || {};
            const values = pair.map((n) => (pct[n] != null ? Number(pct[n]) : null));
            attempts.push({ key, pair, values });
          }
          const hit = attempts.find((a) => a.values.some((v) => v != null && v >= RATE_HIT_MIN));
          const hasAny = attempts.some((a) => a.values.some((v) => v != null));
          const category = hit ? 'green' : hasAny ? 'red' : '';
          const detail = attempts.length
            ? attempts.map((a) => `${a.key} ${a.pair[0]}-${a.pair[1]}: ${a.values.map((v) => (v == null ? '\u2014' : v)).join('/')}`).join(', ')
            : 'no model pairs/rates';
          if (category) {
            await archiveStore.updateSnapshot(snap.id, { category });
            classified.push({ cityId, dateKey: snap.dateKey, category, attempts });
          }
          decidedIds.add(snap.id); // never re-classified by later cycles
          log(`classified ${cityId} ${snap.dateKey} -> ${category || 'no data (unclassified)'} (${detail})`);
        }
      } catch (error) {

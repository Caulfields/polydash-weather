...
//
// How it works (rule confirmed by Igor, 01.09.2026):
...
//   snapshot". Its position is the temperature pair of the basic/auto model
//   slot: two consecutive integers starting at roundHalfDown(todayMax).
//   Manual wd1 refreshes are stamped with their own time and can duplicate a
//   slot, so everything here works on collectedAt chronology, not slot labels.
...
// - The LAST scheduled slot is the "control slot". When the control archive
//   appears, the day's snapshot is classified by comparing temperature pairs:
//   the basic/auto todayMax (rounded the same way) in the control collection
//   EQUALS the initial pair  -> 'green' (предсказатель совпал),
//   anything else            -> 'red'. If the control archive has no basic
//   temperature at all (slots missing), the snapshot is left unclassified.
...
function basicSlot(archive, city) {
...
// The temperature pair of the basic/auto slot: roundHalfDown(todayMax).
// wd1's rates are a pair of consecutive integers starting at this value, so
// the pair is fully described by the rounded temperature alone.
// Same rounding as wd1: .5 rounds DOWN (23.5 -> 23), otherwise nearest.
function positionFromArchive(archive, city) {
  const basic = basicSlot(archive, city);
  const t = basic ? Number(basic.todayMax) : NaN;
  if (!Number.isFinite(t)) return null;
  const floor = Math.floor(t);
  return t - floor === 0.5 ? floor : Math.round(t);
}
...
function controlArchive(day, schedule, city) {
  ...
  if (exact && basicSlot(exact, city)) return exact;
    ...
    for (const cityId of autoCities) {
      ...
      try {
        ...
        for (const snap of snaps) {
          ...
          const initialTemp = initial ? positionFromArchive(initial, city) : null;
          if (initialTemp == null) {
            decidedIds.add(snap.id);
            log(`${cityId} ${snap.dateKey}: initial collection has no basic temperature — left unclassified`);
            continue;
          }
          ...
          const controlTemp = positionFromArchive(control, city);
          if (controlTemp == null) {
            decidedIds.add(snap.id);
            log(`${cityId} ${snap.dateKey}: control collection has no basic temperature — left unclassified`);
            continue;
          }
          const category = controlTemp === initialTemp ? 'green' : 'red';
          await archiveStore.updateSnapshot(snap.id, { category });
          decidedIds.add(snap.id); // never re-classified by later cycles
          classified.push({ cityId, dateKey: snap.dateKey, category, initialTemp, controlTemp });
          log(
            `classified ${cityId} ${snap.dateKey} -> ${category} ` +
            `(initial ${initialTemp}\u00B0, control ${controlTemp}\u00B0)`,
          );
        }
      } catch (error) {
